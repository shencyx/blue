...
unpack_dict(lua_State *L, struct bson_reader *br, bool array) {
	...
	struct bson_reader t = { (uint8_t *)bytes, sz-5 };
...
ltimestamp(lua_State *L) {
	...
	if (lua_isnoneornil(L,2)) {
		...
		uint32_t i = (uint32_t)luaL_checkint(L,2);
...
lsubtype(lua_State *L, int subtype, const uint8_t * buf, size_t sz) {
	...
	case BSON_TIMESTAMP: {
		...
		lua_pushnumber(L, ts[1]);
		//lua_pushunsigned(L, ts[0]);
		lua_pushnumber(L, ts[0]);
		return 3;
	}
	case BSON_REGEX: {
		--sz;
...
typeclosure(lua_State *L) {
	...
	int i;
	int n = sizeof(tn)/sizeof(tn[0]);
	for (i=0;i<n;i++) {
		lua_pushstring(L,tn[i]);
	}
