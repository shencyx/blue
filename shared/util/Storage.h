	...
	public:
		...
		void Load(const char* IndexName, const char* FormatString)
		{
			...
			if(Storage<T, StorageType>::_storage.NeedsMax())
			{
				...
				if(result)
				{
					...
					{
						sLog.Error("Storage", "The table, '%s', has a maximum entry of %u, which is less %u. Any items higher than %u will not be loaded.",
			...
			if(result->GetFieldCount() != cols)
			{
				if(result->GetFieldCount() > cols)
				{
					sLog.Error("Storage", "Invalid format in %s (%u/%u), loading anyway because we have enough data", IndexName, (unsigned int)cols, (unsigned int)result->GetFieldCount());
				}
				else
				{
					sLog.Error("Storage", "Invalid format in %s (%u/%u), not enough data to proceed.", IndexName, (unsigned int)cols, (unsigned int)result->GetFieldCount());
					delete result;
					return;
				}
			}
			...
			sLog.Success("Storage", "%u entries loaded from table %s.", result->GetRowCount(), IndexName);
			delete result;
		...
		void LoadAdditionalData(const char* IndexName, const char* FormatString)
		{
			...
			if(result->GetFieldCount() != cols)
			{
				if(result->GetFieldCount() > cols)
				{
					sLog.Warning("Storage", "Invalid format in %s (%u/%u), loading anyway because we have enough data", IndexName, (unsigned int)cols, (unsigned int)result->GetFieldCount());
				}
				else
				{
					sLog.Error("Storage", "Invalid format in %s (%u/%u), not enough data to proceed.", IndexName, (unsigned int)cols, (unsigned int)result->GetFieldCount());
		...
		void Reload()
		{
			sLog.Notice("Storage", "Reloading database cache from `%s`...", Storage<T, StorageType>::_indexName);
