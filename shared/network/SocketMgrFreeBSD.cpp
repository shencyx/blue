...
void SocketMgr::AddSocket(Socket* s)
{
	...
	{
		sLog.Error("kqueue", "Could not add initial kevent for fd %u!", s->GetFd());
		return;
	}
}
...
void SocketMgr::RemoveSocket(Socket* s)
{
	...
	{
		/* already removed */
		sLog.Warning("kqueue", "Duplicate removal of fd %u!", s->GetFd());
		return;
	}
	fds[s->GetFd()] = 0;
	...
	if(kevent(kq, &ev, 1, 0, 0, NULL) && kevent(kq, &ev2, 1, 0, 0, NULL))
		sLog.Warning("kqueue", "Could not remove from kqueue: fd %u", s->GetFd());
}
...
bool SocketWorkerThread::run()
{
	...
	while(running)
	{
		...
		for(i = 0; i < fd_count; ++i)
		{
			...
			{
				sLog.Warning("kqueue", "Requested FD that is too high (%u)", events[i].ident);
				continue;
			}
			...
			if(ptr == NULL)
			{
				...
				{
					sLog.Warning("kqueue", "Returned invalid fd (no pointer) of FD %u", events[i].ident);
