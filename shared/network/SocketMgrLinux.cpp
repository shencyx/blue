...
void SocketMgr::AddSocket(Socket* s)
{
	...
	if(epoll_ctl(epoll_fd, EPOLL_CTL_ADD, ev.data.fd, &ev))
		sLog.Error("epoll", "Could not add event to epoll set on fd %u", ev.data.fd);
}
...
void SocketMgr::RemoveSocket(Socket* s)
{
	if(fds[s->GetFd()] != s)
	{
		sLog.Error("epoll", "Could not remove fd %u from the set due to it not existing?", s->GetFd());
		return;
	}
	...
	if(epoll_ctl(epoll_fd, EPOLL_CTL_DEL, ev.data.fd, &ev))
		sLog.Error("epoll", "Could not remove fd %u from epoll set, errno %u", s->GetFd(), errno);
}
...
bool SocketWorkerThread::run()
{
	...
	while(running)
	{
		...
		for(i = 0; i < fd_count; ++i)
		{
			...
			{
				sLog.Error("epoll", "Requested FD that is too high (%u)", events[i].data.fd);
				continue;
			}
			...
			if(ptr == NULL)
			{
				if((ptr = ((Socket*)mgr->listenfds[events[i].data.fd])) != NULL)
					...
					sLog.Error("epoll", "Returned invalid fd (no pointer) of FD %u", events[i].data.fd);
