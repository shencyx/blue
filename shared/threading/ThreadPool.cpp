...
bool CThreadPool::ThreadExit(Thread* t)
{
	...
	if (_printDebug)
		sLog.Debug("ThreadPool", "Thread %u entered the free pool.", t->ControlInterface.GetId());
	_mutex.Release();
	return true;
}
...
void CThreadPool::ExecuteTask(ThreadBase* ExecutionTarget)
{
	...
	if(m_freeThreads.size())
	{
		...
		if (_printDebug)
			sLog.Debug("ThreadPool", "Thread %u left the thread pool.", t->ControlInterface.GetId());
	}
	else
	{
	...
	if (_printDebug)
#ifdef WIN32
		sLog.Debug("ThreadPool", "Thread %u is now executing task at 0x%p.", t->ControlInterface.GetId(), ExecutionTarget);
#else
		sLog.Debug("ThreadPool", "Thread %u is now executing task at %p.", t->ControlInterface.GetId(), ExecutionTarget);
#endif
...
void CThreadPool::Startup()
{
	...
	for(i = 0; i < tcount; ++i)
		StartThread(NULL);
	if (_printDebug)
		sLog.Debug("ThreadPool", "Startup, launched %u threads.", tcount);
}

void CThreadPool::ShowStats()
{
	_mutex.Acquire();
	sLog.Debug("ThreadPool", "============ ThreadPool Status =============");
	sLog.Debug("ThreadPool", "Active Threads: %u", m_activeThreads.size());
	sLog.Debug("ThreadPool", "Suspended Threads: %u", m_freeThreads.size());
	sLog.Debug("ThreadPool", "Requested-To-Freed Ratio: %.3f%% (%u/%u)", float(float(_threadsRequestedSinceLastCheck + 1) / float(_threadsExitedSinceLastCheck + 1) * 100.0f), _threadsRequestedSinceLastCheck, _threadsExitedSinceLastCheck);
	sLog.Debug("ThreadPool", "Eaten Count: %d (negative is bad!)", _threadsEaten);
	sLog.Debug("ThreadPool", "============================================");
	_mutex.Release();
}
...
void CThreadPool::IntegrityCheck()
{
	...
	if(gobbled < 0)
	{
		...
		sLog.Debug("ThreadPool", "IntegrityCheck: (gobbled < 0) Spawning %u threads.", new_threads);
	}
	else if(gobbled < THREAD_RESERVE)
	{
		...
		sLog.Debug("ThreadPool", "IntegrityCheck: (gobbled <= 5) Spawning %u threads.", new_threads);
	}
	else if(gobbled > THREAD_RESERVE)
	{
		...
		sLog.Debug("ThreadPool", "IntegrityCheck: (gobbled > 5) Killing %u threads.", kill_count);
	}
	else
	{
		// perfect! we have the ideal number of free threads.
		sLog.Debug("ThreadPool", "IntegrityCheck: Perfect!");
	}
...
{
	if (_printDebug)
		sLog.Debug("ThreadPool", "Killing %u excess threads.", count);
	_mutex.Acquire();
	Thread* t;
...
void CThreadPool::Shutdown()
{
	...
	sLog.Debug("ThreadPool", "Shutting down %u threads.", tcount);
	...
	for(int i = 0;; i++)
	{
		...
		if(m_activeThreads.size() || m_freeThreads.size())
		{
			...
			sLog.Debug("ThreadPool", "%u active and %u free threads remaining...", m_activeThreads.size(), m_freeThreads.size());
			_mutex.Release();
			shared::Sleep(1000);
			continue;
		}
...
static unsigned long WINAPI thread_proc(void* param)
{
	...
	if (ThreadPool.PrintDebug())
		sLog.Debug("ThreadPool", "Thread %u started.", t->ControlInterface.GetId());
	...
	for(;;)
	{
		...
		{
			if(ht && ThreadPool.PrintDebug())
				sLog.Debug("ThreadPool", "Thread %u waiting for a new task.", tid);
