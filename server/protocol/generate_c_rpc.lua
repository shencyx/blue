...
_out_name = _rpc_File_name .. "_c_rpc.lua"
if _output_path_c ~= nil then
	_out_name = _output_path_c .. "/" .. _out_name
end
print("rpc:", _rpc_name, _rpc_File_name, _service_name)
...
function writeInit(outFile)
	...
	local L4 = '\t_protobuf.register_file("generate/%s.pb")\n'
	L4 = string.format(L4, _rpc_File_name)
	...
	local L6 = '\tself.funcList[%d] = {"%s", "%s"}\n'
	local L7 = '\tself.funcList[%d] = {"%s", nil}\n'
	--local L8 = '\tself.funcList[%d] = {"%s", nil, nil}\n'
	--local L9 = '\tself.funcList[%d] = {"%s", nil, "%s"}\n'
	local str = nil
	for i = 1, #classT do
		local entry = classT[i]
		if entry[1] == "void" then
			str = string.format(L7, i, entry[2] .. "Back")
		else
			str = string.format(L6, i, entry[2] .. "Back", entry[1])
		end
		outFile:write(str)
	end
...
function writeDispatch(outFile)
	...
	local L3 = '\tif funcName[2] then\n'
	local L4 = '\t\tmsg, errorStr = _protobuf.decode(funcName[2], buffer, length)\n'
	local L5 = '\tend\n'
	local L6 = '\tif errorStr then\n'
	local L7 = '\t\tprint("error %s:", errorStr, roleId, funcId, length)\n'
	L7 = string.format(L7, className)
	local L8 = '\telse\n'
	local L9 = '\t\tself[funcName[1]](self, roleId, msg)\n'
	local L10 = '\tend\n'
...
function writeFunc(outFile, funcName, parameter, backMsg, operId)
	...
	if parameter == nil or parameter == "void" then
		...
		local L1 = '\t_sendPb(nil, 0, self.operIdFix + %d, roleId)\n'
		L1 = string.format(L1, operId)
		outFile:write(L1)
	else
		writeFuncHead(outFile, funcName, "roleId", "msg")
		local L1 = '\tbuf = _protobuf.encode("%s", msg)\n'
		L1 = string.format(L1, parameter)
		local L2 = '\t_sendPb(buf, string.len(buf), self.operIdFix + %d, roleId)\n'
		L2 = string.format(L2, operId)
		outFile:write(L1)
		outFile:write(L2)
	end
	writeFuncEnd(outFile)
	...
	if backMsg and backMsg ~= "void" then
		writeFuncHead(outFile, funcName .. "Back", "roleId", "msg")
		local L3 = '\terror("%s:%s is not implementation!")\n'
		L3 = string.format(L3, className, funcName)
		outFile:write(L3)
		writeFuncEnd(outFile)
	end
end
